...
class Context:
    ...
    @classmethod
    def set_connection_type(cls, connection_type: type[ABCConnection]) -> type['Context']:
    ...
    @classmethod
    def set_handler(cls, name: str, fn: t.Callable[..., t.Coroutine]) -> type['Context']:
    ...
    def get_handler(cls, name: str) -> t.Callable[..., t.Coroutine]:
        ...
        for i in Context._middlewares:
            handler = ft.update_wrapper(ft.partial(i, handler), handler)

        return handler

    @classmethod
    def add_middleware(cls, fn: t.Callable[..., t.Coroutine]) -> type['Context']:
